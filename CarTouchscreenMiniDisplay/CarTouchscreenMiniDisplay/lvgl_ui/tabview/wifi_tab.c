...
static void sw_event_handler(lv_event_t *e)
{
    ...
    {
        if (lv_obj_has_state(sw_wifi_sta, LV_STATE_CHECKED) && lv_obj_has_state(sw_wifi_ap, LV_STATE_CHECKED) )
            wifi_mode = 3;
        else if (!lv_obj_has_state(sw_wifi_sta, LV_STATE_CHECKED) && lv_obj_has_state(sw_wifi_ap, LV_STATE_CHECKED) )
            wifi_mode = 2;
        else if (lv_obj_has_state(sw_wifi_sta, LV_STATE_CHECKED) && !lv_obj_has_state(sw_wifi_ap, LV_STATE_CHECKED) )
            wifi_mode = 1;
        else
